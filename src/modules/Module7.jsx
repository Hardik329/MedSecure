...
import { ProgressBar } from "../components/ProgressBar";
import AccessControlDiagram from "../components/AccessControlDiagram";
import HandsOn from "../components/HandsOn"; // 🔥 Added HandsOn import
...
const Module7 = () => {
  ...
  const tasks = [
    {
      label:
        "Using the resources folder, find and list all files accessible only to the 'admin' user by examining file permissions.",
      flag: "secret.txt",
    },
    {
      label:
        "Modify the permissions of 'public.txt' file so that only the owner has read and write access. Provide the chmod command used as the flag.",
      flag: "chmod 600 public.txt",
    },
    {
      label:
        "Identify which users belong to the 'doctors' group by listing group memberships. Enter the username of the second user as the flag.",
      flag: "dr_smith",
    },
  ];
  ...
  return (
    <div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-10 text-gray-800">
        ...
        {/* 🔥 Hands-On Section Added Below */}
        <HandsOn
          tasks={tasks}
          title="In this challenge we will practice applying access control concepts by exploring file permissions and user group management."
        />
